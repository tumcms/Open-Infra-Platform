...
static const float tipStart = 0.9;
static const float tipWidth = 4;

VertexToGeometry VS_main(ApplicationToVertex app2vs)
{
	VertexToGeometry vs2gs = (VertexToGeometry)0;
	
	vs2gs.position.xyz = mul((float3x3)view, app2vs.position);
	vs2gs.position.z += 3;
	vs2gs.position.w = 1;

	float4x4 proj = projection;
	proj[0][0] = proj[1][1];
	vs2gs.position = mul(proj, vs2gs.position);
...
void GS_main(line VertexToGeometry vs2gs[2], inout TriangleStream<GeometryToPixel> outputStream)
{
	...
	float4 p01 = p0 + (p1 - p0) * tipStart;

	generateLine(p0, p01, color0, color1, size0, size1, outputStream);
	generateLine(p01, p1, color0, color1, size1 * tipWidth, size1, outputStream);
}
