...
bool readFile(
  ...
  for (std::list<carve::input::Data *>::const_iterator i = inputs.input.begin(); i != inputs.input.end(); ++i) {
    carve::poly::Polyhedron *poly = inputs.create<carve::poly::Polyhedron>(*i, carve::input::Options());
    if (poly) {
      result = poly;
      return true;
    }
...
bool readFile(
  ...
  for (std::list<carve::input::Data *>::const_iterator i = inputs.input.begin(); i != inputs.input.end(); ++i) {
    carve::mesh::MeshSet<3> *poly = inputs.create<carve::mesh::MeshSet<3> >(*i, carve::input::Options());
    if (poly) {
      result = poly;
      return true;
    }
