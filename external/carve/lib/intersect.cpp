...
namespace {
  ...
  inline void facesForVertex(carve::mesh::MeshSet<3>::vertex_t *v,
    ...
    if (vi != ve.end()) {
      for (auto i = (*vi).second.begin(); i != (*vi).second.end(); ++i) {
        faces.insert((*i)->face);
      }
    }
  }
...
static void checkFaceLoopIntegrity(carve::csg::FaceLoopList &fll) {
  ...
  std::unordered_map<carve::csg::V2, int, carve::mesh::hash_vertex_pair> counts;
  ...
  for (std::unordered_map<carve::csg::V2, int, carve::mesh::hash_vertex_pair>::const_iterator
...
#if defined(CARVE_DEBUG_WRITE_PLY_DATA)
  {
    std::shared_ptr<carve::mesh::MeshSet<3> > poly(faceLoopsToPolyhedron(a_face_loops));
    writePLY("/tmp/a_split.ply", poly.get(), false);
  }
  {
    std::shared_ptr<carve::mesh::MeshSet<3> > poly(faceLoopsToPolyhedron(b_face_loops));
