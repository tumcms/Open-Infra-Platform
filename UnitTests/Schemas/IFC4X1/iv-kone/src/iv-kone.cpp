...
class IvKone : public IfcVisualTest {
protected:
	...
	virtual std::string TestName() const { return "iv-kone"; }
	virtual std::string Schema() const { return "IFC4X1"; }

	const boost::filesystem::path filename = dataPath("iv-kone.ifc");

	std::shared_ptr<oip::EXPRESSModel> express_model = nullptr;
	buw::ReferenceCounted<oip::IfcImporterT<emt::IFC4X1EntityTypes>> importer = nullptr;
};

TEST_F(IvKone, AllEntitiesAreRead) {
	EXPECT_THAT(express_model->entities.size(), Eq(28979));
}

TEST_F(IvKone, IFCHasAnEssentialEntity) {
	...
	auto result2 = std::find_if(express_model->entities.begin(), express_model->entities.end(), [](auto &pair) -> bool { return pair.second->classname() == "IFCFACETEDBREP"; });
	
	EXPECT_NE(result, express_model->entities.end());
	EXPECT_NE(result2, express_model->entities.end());
}

TEST_F(IvKone, CountEssentialEntities) {
	auto result = std::count_if(express_model->entities.begin(), express_model->entities.end(), [](auto &pair) -> bool { return pair.second->classname() == "IFCFACETEDBREP"; });

	EXPECT_EQ(result, 5);
}

TEST_F(IvKone, ImageIsSaved)
{
	// Arrange
	buw::Image4b image = rendererIfc->captureImage();

	// Act
	buw::storeImage(testPath("iv-kone.png").string(), image);

	// Assert
	EXPECT_NO_THROW(buw::loadImage4b(testPath("iv-kone.png").string()));
}

TEST_F(IvKone, PlaneSurfaceViews)
{
	// Arrange
	const auto expected_front = buw::loadImage4b(dataPath("iv-kone_front.png").string());
	const auto expected_top = buw::loadImage4b(dataPath("iv-kone_top.png").string());
	const auto expected_bottom = buw::loadImage4b(dataPath("iv-kone_bottom.png").string());
	const auto expected_left = buw::loadImage4b(dataPath("iv-kone_left.png").string());
	const auto expected_right = buw::loadImage4b(dataPath("iv-kone_right.png").string());
	const auto expected_back = buw::loadImage4b(dataPath("iv-kone_back.png").string());
	...
	/*
	buw::storeImage(testPath("iv-kone_front.png").string(), image_front);
	buw::storeImage(testPath("iv-kone_top.png").string(), image_top);
	buw::storeImage(testPath("iv-kone_bottom.png").string(), image_bottom);
	buw::storeImage(testPath("iv-kone_left.png").string(), image_left);
	buw::storeImage(testPath("iv-kone_right.png").string(), image_right);
	buw::storeImage(testPath("iv-kone_back.png").string(), image_back);
	*/
...
TEST_F(IvKone, VertexViews)
{
	// Arrange
	const auto expected_front_left_bottom = buw::loadImage4b(dataPath("iv-kone_front_left_bottom.png").string());
	const auto expected_front_right_bottom = buw::loadImage4b(dataPath("iv-kone_front_right_bottom.png").string());
	const auto expected_top_left_front = buw::loadImage4b(dataPath("iv-kone_top_left_front.png").string());
	const auto expected_top_front_right = buw::loadImage4b(dataPath("iv-kone_top_front_right.png").string());
	const auto expected_top_left_back = buw::loadImage4b(dataPath("iv-kone_top_left_back.png").string());
	const auto expected_top_right_back = buw::loadImage4b(dataPath("iv-kone_top_right_back.png").string());
	const auto expected_back_left_bottom = buw::loadImage4b(dataPath("iv-kone_back_left_bottom.png").string());
	const auto expected_right_bottom_back = buw::loadImage4b(dataPath("iv-kone_right_bottom_back.png").string());
	...
	// uncomment following lines to also save the screen shot
	/*
	buw::storeImage(testPath("iv-kone_front_left_bottom.png").string(), image_front_left_bottom);
	buw::storeImage(testPath("iv-kone_front_right_bottom.png").string(), image_front_right_bottom);
	buw::storeImage(testPath("iv-kone_top_left_front.png").string(), image_top_left_front);
	buw::storeImage(testPath("iv-kone_top_front_right.png").string(), image_top_front_right);
	buw::storeImage(testPath("iv-kone_top_left_back.png").string(), image_top_left_back);
	buw::storeImage(testPath("iv-kone_top_right_back.png").string(), image_top_right_back);
	buw::storeImage(testPath("iv-kone_back_left_bottom.png").string(), image_back_left_bottom);
	buw::storeImage(testPath("iv-kone_right_bottom_back.png").string(), image_right_bottom_back);
	*/
