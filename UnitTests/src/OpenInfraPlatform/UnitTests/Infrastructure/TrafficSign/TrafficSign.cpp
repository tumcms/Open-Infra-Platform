...
namespace {
	...
	class TrafficSignTestFileGenerator {
		shared_ptr<IfcOwnerHistory> ownerHistory;
		shared_ptr<IfcProject> project;
		shared_ptr<IfcSite> site;

	public:
		TrafficSignTestFileGenerator(const std::string& filename) {
			// create model
			...
			shared_ptr<IfcAxis2Placement3D> worldCoordinateSystem = createIfcAxis2Placement3D(createIfcCartesianPoint(0, 0, 0));
			
			geometricRepresentationContext->m_WorldCoordinateSystem = worldCoordinateSystem;
			...
			if (subcontextAxis == nullptr) {
				...
				//subcontextAxis->m_TargetScale
				subcontextAxis->m_TargetView = std::make_shared<IfcGeometricProjectionEnum>(IfcGeometricProjectionEnum::ENUM_MODEL_VIEW);
				//subcontextAxis->m_UserDefinedTargetView
				subcontextAxis->m_WorldCoordinateSystem = worldCoordinateSystem;
				...
				auto subcontextBody = std::make_shared<IfcGeometricRepresentationSubContext>(createEntityId());
				model_->insertEntity(subcontextBody);

				subcontextBody->m_ContextIdentifier = std::make_shared<IfcLabel>("Body");
				subcontextBody->m_ContextType = std::make_shared<IfcLabel>("Model");
				subcontextBody->m_ParentContext = geometricRepresentationContext;
				//subcontextBody->m_TargetScale
				subcontextBody->m_TargetView = std::make_shared<IfcGeometricProjectionEnum>(IfcGeometricProjectionEnum::ENUM_MODEL_VIEW);
				//subcontextBody->m_UserDefinedTargetView
				subcontextBody->m_WorldCoordinateSystem = worldCoordinateSystem;
				subcontextBody->m_CoordinateSpaceDimension = std::make_shared<IfcDimensionCount>(3);
				subcontextBody->m_Precision = std::make_shared<IfcReal>(1e-5);
			
			shared_ptr<IfcBuilding> building = std::make_shared<IfcBuilding>(createEntityId());
			model_->insertEntity(building);
			building->m_GlobalId = createGlobalId();
			building->m_OwnerHistory = ownerHistory;
			building->m_Name = std::make_shared<IfcLabel>("IfcBuilding");
			...
			ownerHistory = createIfcOwnerHistory(pao, application);


			wirteStepFile(filename);
		}

		TrafficSignTestFileGenerator(const ifcAlignmentExportDescription& desc,
			buw::ReferenceCounted<buw::AlignmentModel> am,
			buw::ReferenceCounted<buw::DigitalElevationModel> dem,
			const std::string& filename) : alignmentModel_(am), digitalElevationModel_(dem) {
			// create model
			model_ = std::shared_ptr<IfcAlignment1x1Model>(new IfcAlignment1x1Model());
			...
			try {
				...
				ownerHistory = createIfcOwnerHistory(personAndOrganization, application);
				...
				// create project
				project = std::make_shared<IfcProject>(createEntityId());
				...
				site = std::make_shared<IfcSite>(createEntityId());
				...
				exportTrafficSign(site_axisPlacement, geometricRepresentationContext, buw::Vector3d(0.0, 0.0, 0.0), c2);
		...
		std::shared_ptr<IfcAlignment> a1_alignment;

		shared_ptr<IfcDirection> createIfcDirection(buw::Vector3d v) {
			return createIfcDirection(v.x(), v.y(), v.z());
		}

		shared_ptr<IfcDirection> createIfcDirection(double x, double y, double z) {
			shared_ptr<IfcDirection> dir = make_shared<IfcDirection>(createEntityId());
			model_->insertEntity(dir);
			dir->m_DirectionRatios.push_back(std::make_shared<IfcReal>(x));
			dir->m_DirectionRatios.push_back(std::make_shared<IfcReal>(y));
			dir->m_DirectionRatios.push_back(std::make_shared<IfcReal>(z));
			return dir;
		}

		void exportTrafficSign(shared_ptr<IfcAxis2Placement3D> site_axisPlacement,
		...
		{ 
			std::string inputfile = "Data/TrafficSign2.obj";
			...
			for (int pointIndex = 0; pointIndex < shapes[0].mesh.positions.size() / 3; pointIndex++) {

				double x = shapes[0].mesh.positions[3*pointIndex+0] - centerOffset.x();
				double y = shapes[0].mesh.positions[3*pointIndex+1] - centerOffset.y();
				double z = shapes[0].mesh.positions[3*pointIndex+2] - centerOffset.z();

				std::vector<shared_ptr<IfcLengthMeasure>> coordinates;
				coordinates.push_back(std::make_shared<IfcLengthMeasure>(x));
				coordinates.push_back(std::make_shared<IfcLengthMeasure>(z)); // y und x vertauschen weil OBJ Koordinatensystem anders
			...
			for (int faceIndex = 0; faceIndex < shapes[0].mesh.indices.size() / 3; faceIndex++) {
				std::vector<std::shared_ptr<IfcPositiveInteger>> face_indices;

				// reverse order of faces
				face_indices.push_back(std::make_shared<IfcPositiveInteger>(shapes[0].mesh.indices[3*faceIndex+0]+1));
				face_indices.push_back(std::make_shared<IfcPositiveInteger>(shapes[0].mesh.indices[3*faceIndex+1]+1));
				face_indices.push_back(std::make_shared<IfcPositiveInteger>(shapes[0].mesh.indices[3*faceIndex+2]+1));
			...
			shared_ptr<IfcDistanceExpression> de = std::make_shared<IfcDistanceExpression>(createEntityId());
			model_->insertEntity(de);
			de->m_DistanceAlong = std::make_shared<IfcLengthMeasure>(0);
			de->m_OffsetLateral = std::make_shared<IfcLengthMeasure>(-5);
			//de->m_OffsetVertical = std::make_shared<IfcLengthMeasure>(0);
			//de->m_OffsetLongitudinal = std::make_shared<IfcLengthMeasure>(0);
			de->m_AlongHorizontal = std::make_shared<IfcBoolean>(true);

			shared_ptr<IfcOrientationExpression> oe = std::make_shared<IfcOrientationExpression>(createEntityId());
			model_->insertEntity(oe);
			oe->m_VerticalAxisDirection = createIfcDirection(buw::Vector3d(0, 0, 1));
			
			buw::Vector4d standardLateralDirection(0, 1, 0, 0);
			buw::Vector4d lateralDirection = buw::createRotationMatrixZ(buw::degreeToRadian(190.0 +60)) * standardLateralDirection;
			std::cout << lateralDirection << std::endl;
			oe->m_LateralAxisDirection = createIfcDirection(lateralDirection.x(), lateralDirection.y(), lateralDirection.z());

			shared_ptr<IfcLinearPlacement> lp = std::make_shared<IfcLinearPlacement>(createEntityId());
			model_->insertEntity(lp);
			lp->m_PlacementRelTo = a1_alignment->m_Axis;
			lp->m_Distance = de;
			lp->m_Orientation = oe;
			

			shared_ptr<IfcBuildingElementProxy> proxy = std::make_shared<IfcBuildingElementProxy>(createEntityId());
			model_->insertEntity(proxy);
			proxy->m_GlobalId = createGlobalId();
			proxy->m_OwnerHistory = ownerHistory;
			proxy->m_Name = std::make_shared<IfcLabel>("Traffic sign");
			proxy->m_Description = std::make_shared<IfcText>("Speed limit traffic sign");
			proxy->m_ObjectType = std::make_shared<IfcLabel>("");
			proxy->m_ObjectPlacement = lp;
			proxy->m_Representation = createIfcProductDefinitionShape(representation);

			//proxy->m_Tag
			proxy->m_PredefinedType = std::make_shared<IfcBuildingElementProxyTypeEnum>(IfcBuildingElementProxyTypeEnum::ENUM_USERDEFINED);
			...
			c2->m_RelatedElements.push_back(proxy);

			//shared_ptr<IfcRelContainedInSpatialStructure> relContainedSS = std::make_shared<IfcRelContainedInSpatialStructure>(createEntityId());
			//model_->insertEntity(relContainedSS);
			//relContainedSS->m_GlobalId = createGlobalId();
			//relContainedSS->m_OwnerHistory = ownerHistory;
			//relContainedSS->m_Name = std::make_shared<IfcLabel>("Building");
			//relContainedSS->m_Description = std::make_shared<IfcText>("Building Container for Elements");
			//relContainedSS->m_RelatedElements.push_back(proxy);
			//relContainedSS->m_RelatingStructure = site;

			//shared_ptr<IfcRelAggregates> relAggregates = std::make_shared<IfcRelAggregates>(createEntityId());
			//model_->insertEntity(relAggregates);
			//relAggregates->m_GlobalId = createGlobalId();
			//relAggregates->m_OwnerHistory = ownerHistory;
			//relAggregates->m_Name = std::make_shared<IfcLabel>("Project Container");
			//relAggregates->m_Description = std::make_shared<IfcText>("Project Container for Buildings");
			//relAggregates->m_RelatingObject = project;
			//relAggregates->m_RelatedObjects.push_back(building);
		}

		virtual ~TrafficSignTestFileGenerator() {
		}
	...
	private:
		...
		void convertHorizontalAlignment(std::shared_ptr<OpenInfraPlatform::IfcAlignment1x1::IfcAlignmentCurve> ifcAlignment,
			...
			for (int i = 0; i < alignment->getHorizontalAlignment()->getAlignmentElementCount(); i++) {
				...
				if (hAlignSegment->getAlignmentType() == buw::eHorizontalAlignmentType::Line) {
					...
					createIfcClothoidalArcSegment2D(clothoid,
		...
		shared_ptr<IfcPerson> createIfcPerson() {
			...
			person->m_FamilyName = std::make_shared<IfcLabel>("Julian");
			person->m_GivenName = std::make_shared<IfcLabel>("Amann");
	...
	TEST(TrafficSignTestFileGenerator, CreateFile) {
		buw::loadWebResources("Data/resources.xml");

		//buw::ImportIfcAlignment1x1 ifcImport("C:/Users/admin/OneDrive/dev/OpenInfraPlatfom/testdata/Ifc2x3/Houses/ADT-FZK-Haus-2005-2006.ifc");


		buw::ImportLandXml landXMLImporter("C:/build/vs2015/x64/OpenInfraPlatform2/testdata/LandXML/Mainbruecke_Klingenberg.xml");

		int alignmentCount1 = landXMLImporter.getAlignmentModel()->getAlignmentCount();
	
		ifcAlignmentExportDescription desc;
	
		//PlacementTestFileGenerator p("out.ifc");
		TrafficSignTestFileGenerator p(desc, landXMLImporter.getAlignmentModel(), landXMLImporter.getDigitalElevationModel(), "C:/dev/OpenInfraPlatform2/IFC4x1ExampleFiles/OIP Revision 1837/E.1.5 Linear Placement/linear-placement-ifc-alignment-1x1.ifc");
