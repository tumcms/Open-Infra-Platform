				...
				if (bounded_surface) {
					...
					if (bspline_surface) {
						...
						for (auto& it_control_point_list : vec_control_point_list) {
							...
							std::transform(it_control_point_list.begin(), it_control_point_list.end(), vec_control_point_list.begin(), [](auto& it) { return it.lock(); });
						}
						...
						if (bspline_knots) {
							...
							OpenInfraPlatform::IFC4X1::IfcKnotType knot_type = bspline_knots->KnotSpec;
