...
#pragma once
#ifndef INCONSISTENT_GEOMETRY_EXCEPTION_H
#define INCONSISTENT_GEOMETRY_EXCEPTION_H
...
*/
class InconsistentGeometryException : public std::exception 
{
public:
    InconsistentGeometryException() noexcept;
	...
	*/
    template <typename T>
    InconsistentGeometryException(const oip::EXPRESSReference<typename T>& item, const std::string& message) noexcept
    {
        item_ = item.lock();
        message_ = message;
		init();
...
EMBED_CORE_EXCEPTION_INTO_OIP_NAMESPACE(InconsistentGeometryException)
