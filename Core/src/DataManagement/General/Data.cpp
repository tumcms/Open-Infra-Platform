...
#include "reader/IFC4X3Reader.h"
#include "EMTIFC4X3EntityTypes.h"
#include "IFC4X3.h"
...
void OpenInfraPlatform::Core::DataManagement::Data::importJob(const std::string& filename)
{
	...
	if (filetype == ".ifc" || filetype == ".stp")
	{
		...
		if (ifcSchema == IfcPeekStepReader::IfcSchema::IFC4X3) {
			expressModel_ = OpenInfraPlatform::IFC4X3::IFC4X3Reader::FromFile(filename);
			auto importer = OpenInfraPlatform::Core::IfcGeometryConverter::IfcImporterT<emt::IFC4X3EntityTypes, OpenInfraPlatform::Core::IfcGeometryConverter::UnitConverter< emt::IFC4X3EntityTypes>>();
			if (importer.collectGeometryData(expressModel_)) {
				auto converter = IfcGeometryConverter::ConverterBuwT<emt::IFC4X3EntityTypes>();
				if (converter.createGeometryModel(tempIfcGeometryModel_, importer.getShapeDatas())) {
					if (!tempIfcGeometryModel_->isEmpty()) {
						ifcGeometryModel_ = tempIfcGeometryModel_;
					}
