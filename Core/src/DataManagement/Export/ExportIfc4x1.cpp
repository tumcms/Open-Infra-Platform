...
#include "DataManagement/Export/ExportIfc4x1.h"
...
public:
	...
	void createModel(const OpenInfraPlatform::Infrastructure::ifcAlignmentExportDescription & desc)
	{
		...
		try {
			...
			projectedCRS->m_Name = std::make_shared<IfcLabel>(desc.m_Name); // todo get EPSG code from Gauï¿½-Krueger coordinates
			projectedCRS->m_Description = nullptr;// std::make_shared<IfcText>("EPSG:31467 - DHDN / 3-Degree Gauss-Krueger Zone 3"); //to do, because not applicable in all cases
			projectedCRS->m_GeodeticDatum = nullptr; // std::make_shared<IfcIdentifier>("EPSG:31467"); //to do, because not applicable in all cases
			projectedCRS->m_MapProjection = nullptr; // std::make_shared<IfcIdentifier>("Gauss-Krueger"); //to do, because not applicable in all cases
			projectedCRS->m_MapZone = nullptr; // std::make_shared<IfcIdentifier>("3");  //to do, because not applicable in all cases
