...
#include "ExportSVGEAdvanced.h"
#include <list>

void OpenInfraPlatform::Infrastructure::ExportSVGEAdvanced::outputToSVG(FILE *fp, buw::ReferenceCounted<buw::HorizontalAlignment2D> horizontalAlignment)
{
	std::string output = "";
	buw::Vector2d start;
...
void OpenInfraPlatform::Infrastructure::ExportSVGEAdvanced::addPath(FILE *fp, const buw::Vector2d& a, const buw::Vector2d& b, const buw::Vector2d& c)
{
	fprintf(fp, "<path d=\"M %f,%f L %f,%f L %f,%f z\"/>\n", a.x(), -a.y(), b.x(), -b.y(), c.x(), -c.y());
}

OpenInfraPlatform::Infrastructure::ExportSVGEAdvanced::ExportSVGEAdvanced(buw::ReferenceCounted<buw::AlignmentModel> am, buw::ReferenceCounted<buw::DigitalElevationModel> dem, const std::string& filename) :
