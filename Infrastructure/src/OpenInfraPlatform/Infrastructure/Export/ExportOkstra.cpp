	...
	void exportHorizontalAlignment(buw::ReferenceCounted<buw::HorizontalAlignment2D> horizontalAlignment, Oklabi::Fachobjekt* achse) {
		...
		for (int i = 0; i < horizontalAlignment->getAlignmentElementCount(); i++) {
			...
			case buw::eHorizontalAlignmentType::Clothoid: {
				...
				double radiusStart;
				element->genericQuery(buw::eAlignmentElementQueryId::RadiusStart, &radiusStart);

				double radiusEnd = 0.0;
				element->genericQuery(buw::eAlignmentElementQueryId::RadiusEnd, &radiusEnd);

				double clothoidConstant;
				element->genericQuery(buw::eAlignmentElementQueryId::ClothoidConstant, &clothoidConstant);

				double startDirection;
				element->genericQuery(buw::eAlignmentElementQueryId::AlignmentStartDirection1D, &startDirection);

				bool cwise;
				element->genericQuery(buw::eAlignmentElementQueryId::Clockwise, &cwise);
...
OpenInfraPlatform::Infrastructure::ExportOkstra::ExportOkstra(int majorVersion,
                                                              int minorVersion,
                                                              buw::ReferenceCounted<buw::AlignmentModel> am,
                                                              buw::ReferenceCounted<buw::DigitalElevationModel> dem,
                                                              const std::string& filename)
    : Export(am, dem, filename), impl_(new OkstraExportImpl(majorVersion, minorVersion, am, dem, filename)) {
}
