...
#include "ExportIfcAlignment1x0.h"
...
class OpenInfraPlatform::Infrastructure::ExportIfcAlignment1x0::IfcAlignment1x0ExportImpl : public Export
{
public:
	IfcAlignment1x0ExportImpl(const ifcAlignmentExportDescription& desc, buw::ReferenceCounted<buw::AlignmentModel> am, buw::ReferenceCounted<buw::DigitalElevationModel> dem, const std::string& filename) :
		Export(am, dem, filename),
		entityId_(1),
		model_(nullptr)
	{
		// create model
		model_ = shared_ptr<IfcAlignmentModel>(new IfcAlignmentModel());
		...
		catch (const IfcAlignmentP6Exception& e)
		{
			std::cout << e.what() << std::endl;
		}
	}

	virtual ~IfcAlignment1x0ExportImpl()
	{
		...
		shared_ptr<IfcRelContainedInSpatialStructure> c2)
	{
		for (int ai = 0; ai < alignments.size(); ai++)
		{
			if (alignments[ai]->getType() == buw::e3DAlignmentType::e3DBased)
			{
				...
				shared_ptr<OpenInfraPlatform::IfcAlignment::IfcAlignment> ifcAlignment =
					std::make_shared<OpenInfraPlatform::IfcAlignment::IfcAlignment>(createEntityId());
			...
			else
			{
				...
				convertVerticalAlignment(alignment, ifcAlignment, centerOffset.z());
	...
	void wirteStepFile(const std::string &filename)
	{
		...
		{
		case  eIfcSchemaVersion::IFC2x3:
			fileSchema = "IFC2X3";
			break;
		case  eIfcSchemaVersion::IFC4:
			fileSchema = "IFC4";
			break;
		case eIfcSchemaVersion::IFC4x1:
			fileSchema = "IFC4X1";
			break;
		default:
			break;
...
private:
		...
		buw::Vector3d &centerOffset)
	{
		...
		for (int i = 0; i < alignment->getHorizontalAlignment()->getAlignmentElementCount(); i++)
		{
			...
			else
			{
				...
				createIfcClothoidalArcSegment2D(clothoid, start - centerOffset.block<2,1>(0,0), end - centerOffset.block<2,1>(0,0), curvatureStart, curvatureEnd, clockwise, length, startDirection, pi - centerOffset.block<2,1>(0,0), clothoidConstant);
			}
		}
	}
		...
		//---------------------------
			...
			else
			{
				...
				for (int faceIndex = 0; faceIndex < s->getTriangleCount(); faceIndex++)
				{
					...
					// reverse order of faces
					face_indices.push_back(s->getTriangeFaces()[faceIndex][0]+1);
					face_indices.push_back(s->getTriangeFaces()[faceIndex][2]+1);
					face_indices.push_back(s->getTriangeFaces()[faceIndex][1]+1);
		...
		const double clothoidConstant)
	{
		...
		else
		{
			...
			curveSegement->m_StartRadius->m_value = std::abs(1.0 / curvatureStart);
		}
	...
	double mapAngleToRatioMeasure(buw::degreef angle)
	{
		if (angle <= 180.0)
		{
			angle = angle / 45.0;
...
private:
	shared_ptr<IfcAlignmentModel>	model_;
	int								entityId_;
	ifcAlignmentExportDescription	settings_;
};

OpenInfraPlatform::Infrastructure::ExportIfcAlignment1x0::ExportIfcAlignment1x0(
	const ifcAlignmentExportDescription& desc, 
	buw::ReferenceCounted<buw::AlignmentModel> am, 
	buw::ReferenceCounted<buw::DigitalElevationModel> dem,
	const std::string& filename) :
Export(am, dem, filename),
impl_(new IfcAlignment1x0ExportImpl(desc, am, dem, filename))
{
