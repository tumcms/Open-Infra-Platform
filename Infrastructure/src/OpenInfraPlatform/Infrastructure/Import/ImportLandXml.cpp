...
namespace OpenInfraPlatform {
    namespace Infrastructure {
        ...
        public:
            ...
            QLandXMLImportDirector(LandXMLDataBuilder* builder) : builder_(builder) {
            ...
            bool startElement(const QString& /* unused */, const QString& name, const QString& /* unused */, const QXmlAttributes& attr) {
                if (name == "Alignment") {
            ...
            bool endElement(const QString& /* unused */, const QString& name, const QString& /* unused */) {
                if (name == "Alignment") {
        ...
        void constructLandXMLData(const std::string& filename, LandXMLDataBuilder* builder) {
        ...
        buw::ReferenceCounted<buw::VerticalAlignment2D> createVerticalAlignmentFromProfAlign(const std::vector<profAlignElement>& profAlignElements_) {
            ...
            for (int i = 2; i < profAlignElements_.size(); i++) {
                ...
                if (b.type == eProfAlignType::PVI) {
                    ...
                    buw::ReferenceCounted<buw::VerticalAlignmentElement2DParabola> p =
                      std::make_shared<buw::VerticalAlignmentElement2DParabola>(pvc, pvt, l1.getGradient(), l2.getGradient());
        ...
        std::vector<profAlignElement> createProfAlignElements(buw::ReferenceCounted<buw::VerticalAlignment2D> v) {
            ...
            buw::Vector2d start;
            v->getAlignmentElementByIndex(0)->genericQuery(OpenInfraPlatform::Infrastructure::eAlignmentElementQueryId::StartPosition, &start);
            ...
            for (int i = 0; i < v->getAlignmentElementCount() - 1; i++) {
                ...
                if (current->getAlignmentType() == buw::eVerticalAlignmentType::Line && next->getAlignmentType() == buw::eVerticalAlignmentType::Line) {
                    buw::Vector2d end;
                    current->genericQuery(OpenInfraPlatform::Infrastructure::eAlignmentElementQueryId::EndPosition, &end);
                ...
                else if (current->getAlignmentType() == buw::eVerticalAlignmentType::Line && next->getAlignmentType() == buw::eVerticalAlignmentType::Parabola) {
                    buw::Vector2d pvi;
                    next->genericQuery(OpenInfraPlatform::Infrastructure::eAlignmentElementQueryId::PVI, &pvi);
                    double intersectionPointDistance = 0;
                    next->genericQuery(OpenInfraPlatform::Infrastructure::eAlignmentElementQueryId::IntersectionPointDistance, &intersectionPointDistance);
            ...
            buw::Vector2d end;
            v->getAlignmentElementByIndex(v->getAlignmentElementCount() - 1)->genericQuery(OpenInfraPlatform::Infrastructure::eAlignmentElementQueryId::EndPosition, &end);
        ...
        QLandXmlParser::QLandXmlParser() : is3DBased_(false) {
        ...
        bool QLandXmlParser::endElement(const QString& /* unused */, const QString& name, const QString& /* unused */) {
            ...
            if (name == "Curve") {
                ...
                buw::ReferenceCounted<buw::HorizontalAlignmentElement2DArc> arc = std::make_shared<buw::HorizontalAlignmentElement2DArc>(center, start, end, clockwise_);
                ...
                if (bUseArbitraryCurve) {
                    ...
                    */
                    ...
                    auto c = buw::makeReferenceCounted<buw::HorizontalAlignmentElement2DIfcPLArbitraryTransitionCurve>(
                      start, startDirection, startCurvature, !clockwise_, clothoidConstant, entry, length_);
                    ...
                    if (spiType_ == "bloss") {
                        ...
                        buw::blossDescription desc(start, startDirection, !clockwise_, entry, length_, radius);
                    ...
                    } else //(spiType_ == "clothoid")
                    {
                        ...
                        buw::clothoidDescription desc(start, startDirection, startCurvature, !clockwise_, clothoidConstant, entry, length_);
        ...
        buw::Vector3d QLandXmlParser::readVector3d(const QString& ch) {
            ...
            if (positions.size() != 3) {
                std::cout << positions[0].toStdString() << " " << positions[1].toStdString() << " " << positions[2].toStdString() << " " << positions[3].toStdString() << " "
                          << std::endl;
        ...
        void QLandXmlParser::createVerticalAlignment() {
            ...
            if (profAlignElements_.size() > 0) {
                if (profAlignElements_[0].type == eProfAlignType::ParaCurve) {
                    BLUE_LOG(warning) << "Detected invalid vertical alignment - start point is ParaCurve - workaround: converting ParaCurve to PVI";
                ...
                if (profAlignElements_[lastElementIndex].type == eProfAlignType::ParaCurve) {
                    BLUE_LOG(warning) << "Detected invalid vertical alignment  - end point is ParaCurve - workaround: converting ParaCurve to PVI";
        ...
        public:
            void handleMessage(QtMsgType type, const QString& descr, const QUrl&, const QSourceLocation& loc) {
                const char* typeName;
                switch (type) {
        ...
        public:
            int createVertexAndGetIndex(const buw::Vector3d& position) {
        ...
        ImportLandXml::ImportLandXml(const std::string& filename) : Import(filename) {
