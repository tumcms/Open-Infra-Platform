					...
					if (bounded_curve)
					{
							...
							// check if already implemented: OIP->OIP.Infrastructure->OIP->Infrastructure->Alignment
						{
							...
							for(; it_segment != horSegments.end(); ++it_segment) 
							{
								...
								std::vector<std::shared_ptr<typename IfcEntityTypesT::IfcPositiveLengthMeasure> > curveSegLength =
							...
							else 
							{
								...
								for(; it_segment != verSegments.end(); ++it_segment) 
								{
									...
									std::vector<std::shared_ptr<typename IfcEntityTypesT::IfcLengthMeasure> > verDistAlong =
									...
									std::vector<std::shared_ptr<typename IfcEntityTypesT::IfcPositiveLengthMeasure> > verHorizontalLength =
									...
									std::vector<std::shared_ptr<typename IfcEntityTypesT::IfcLengthMeasure> > verStartHeight =
									...
									std::vector<std::shared_ptr<typename IfcEntityTypesT::IfcRatioMeasure> > verStartGradient =
									...
									if(v_seg_circ_arc_2D) 
									{
										...
										verStations.push_back(Station(dVerDistAlong, 0, 0, zStart, nVerFragments, Station::isCircArc));
									}

									if(ver_seg_line_2D) 
									{
										nVerFragments = 0;
										verStations.push_back(Station(dVerDistAlong, 0, 0, zStart, nVerFragments, Station::isLine));
									}

									if(v_seg_par_arc_2D) 
									{
										nVerFragments = m_geomSettings->m_min_num_vertices_per_arc;
										verStations.push_back(Station(dVerDistAlong, 0, 0, zStart, nVerFragments, Station::isParArc));
									}
